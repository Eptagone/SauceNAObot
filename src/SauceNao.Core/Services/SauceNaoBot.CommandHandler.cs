...
public partial class SauceNaoBot : AsyncTelegramBotBase<SnaoBotProperties>
{
	...
	private async Task SetLangAsync(string[] args, CancellationToken cancellationToken)
	{
		...
		bool CultureIsValid(string languageCode)
		{
			try
			{
		...
		// Send a message to the user with the default language options.
		...
		// Send a message to the user confirming that the language has been saved.
		...
		if (args.Length > 0)
		{
			...
			if (isPrivate)
			{
				...
				{
					bool force;
					bool.TryParse(args[1], out force);
					if (force)
					{
				...
				else
				{
					...
					switch (sauceResult.Status)
					{
						...
						case SauceStatus.NotFound:
							if (this.Properties.LocalMode) // Local Mode
							{
	...
	private async Task TempAsync(Message message, CancellationToken cancellationToken)
	{
		...
		if (this.Properties.LocalMode) // Temp command is disabled in Local Mode
		{
	...
	/// <summary>
	/// Send a message with the bot statistics.
	/// </summary>
	/// <param name="cancellationToken">The cancellation Token</param>
	/// <returns></returns>
	private async Task StatisticsAsync(CancellationToken cancellationToken)
	{
		...
		// Count the number of sauces, users and groups.
		var sucefullSearchCount = this._db.Sauces.GetAllSauces().Count();
		// Count active users.
		var usersCount = this._db.Users.GetAllUsers()
			.Where(u => u.UserSauces.Any()).Count();
		// Count active groups.
	...
	/// <summary>
	/// Send information about the languages used by the users.
	/// </summary>
	/// <param name="cancellationToken">The cancellation token</param>
	/// <returns></returns>
	private async Task LanguagesAsync(CancellationToken cancellationToken)
	{
		...
		// Get the languages used by the users.
