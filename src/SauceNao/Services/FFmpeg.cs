...
namespace SauceNao.Services
{
    ...
    public sealed class FFmpeg
    {
        ...
        public bool Run(string input, string output, params string[] arguments) => Run($"-i \"{input}\" {string.Join(' ', arguments)} \"{output}\"");

        /// <summary>Run ffmpeg command.</summary>
        /// <param name="arguments">Arguments.</param>
        public bool Run(string arguments) => Run(arguments, out _);
